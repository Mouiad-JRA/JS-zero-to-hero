...
1. Create a method called 'registerNewAnswer' on the 'poll' object. The method does 2 things:
  ...
  increase the value AT POSITION 3 of the array by 1. Make sure to check if the input is a number and if the
...
So what should the keyword look like in this situation?
...
const poll = {
    ...
    registerNewAnswer(){
        let number = Number(prompt(`${this.question} \n ${this.options.join('\n')}\n (Write option number)`, 1));
        if( typeof number === "number" && number< this.answers.length ) this.answers[number]++ ;
        this.displayResults();
    },
    displayResults(type = 'array'){
            typeof type=== "string" ? alert(`Poll results are ${this.answers}`) : console.log(this.answers.join(','));
    }
};
