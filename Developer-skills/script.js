...
const calcTempAmplitude = function (temperature){
    ...
    for (let i=0;i<temperature.length;i++) {
        ...
        validTemperature.push(temperature[i])
    }

    let sorted = validTemperature.sort(function(a, b){return a - b});
    console.log(sorted)
...
*/
const calcTwoTempAmplitude = function (temperature, temp) {
    const temps = temperature.concat(temp);
    let validTemperature = []
    for (let i = 0; i < temps.length; i++){
        if (typeof temps[i] !== 'number') continue;
        validTemperature.push(temps[i])
    }
...
console.log(calcTwoTempAmplitude(temperatures, temperatures))
/*
// 1) Understanding the problem
...
*/
const printForecast = function (arrNumber){
    let arrString = []
    for (let i=0;i<arrNumber.length;i++){
        arrString.push(`${Number(arrNumber[i])}ºC in ${i+1} days  ...`)
    }
    return arrString;
}
